...
serve(async (req) => {
  ...
  try {
    ...
    const { country, page, per_page } = await req.json().catch(() => ({}));
    ...
    const qs: string[] = [];
    if (country) qs.push(`country=${encodeURIComponent(String(country))}`);
    if (page) qs.push(`page=${encodeURIComponent(String(page))}`);
    if (per_page) qs.push(`per_page=${encodeURIComponent(String(per_page))}`);

    const url = `https://api.vobiz.ai/api/v1/account/${encodeURIComponent(VOBIZ_X_AUTH_ID)}/inventory/numbers${qs.length ? `?${qs.join("&")}` : ""}`;
